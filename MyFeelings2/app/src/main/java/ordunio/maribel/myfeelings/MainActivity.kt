package ordunio.maribel.myfeelings
...
import ordunio.maribel.myfeelings.utilities.CustomBarDrawable
import ordunio.maribel.myfeelings.utilities.CustomCircleDrawable
import ordunio.maribel.myfeelings.utilities.Emociones
import ordunio.maribel.myfeelings.utilities.JSONFile
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var guardarBtn: Button = findViewById(R.id.guardarbutton) as Button
        var veryHappyBtn: ImageButton = findViewById(R.id.veryhappyButton) as ImageButton
        var happyBtn: ImageButton = findViewById(R.id.happyButton) as ImageButton
        var neutralBtn: ImageButton = findViewById(R.id.neutralButton) as ImageButton
        var sadBtn: ImageButton = findViewById(R.id.sadButton) as ImageButton
        var verySadBtn: ImageButton = findViewById(R.id.verysadButton) as ImageButton
    ...
    fun iconoMayoria(){
        ...
        if(happy>veryHappy && happy>neutral && happy>sad && happy>verysad){
            icon.setImageDrawable(resources.getDrawable(R.drawable.ic_baseline_sentiment_very_satisfied_24))
        }

        if(veryHappy>happy && veryHappy>neutral && veryHappy>sad && veryHappy>verysad){
            icon.setImageDrawable(resources.getDrawable(R.drawable.ic_baseline_sentiment_satisfied_alt_24))
        }

        if(neutral>veryHappy && neutral>happy && neutral>sad && neutral>verysad){
            icon.setImageDrawable(resources.getDrawable(R.drawable.ic_baseline_sentiment_satisfied_24))
        }

        if(sad>happy && sad>neutral && sad>veryHappy && sad>verysad){
            icon.setImageDrawable(resources.getDrawable(R.drawable.ic_baseline_sentiment_dissatisfied_24))
        }

        if(verysad>happy && verysad>neutral && verysad>sad && veryHappy<verysad){
            icon.setImageDrawable(resources.getDrawable(R.drawable.ic_baseline_sentiment_very_dissatisfied_24))
        }
    }
